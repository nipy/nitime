...
def test_SeedCorrelationAnalyzer():
    ...
    np_coef_array = np.array(map(lambda a: np.corrcoef(seed.data, a)[0,1], targ.data))
    ...
    our_coef_array = corr.corrcoef
    for source in [0, 1]:
        np_coef_array = np.array(map(lambda a: np.corrcoef(seed.data[source], a)[0,1], targ.data))
