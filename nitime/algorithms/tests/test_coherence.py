...
def test_coherency_regularized():
    ...
    for method in methods:
        f,c = tsa.coherency_regularized(tseries,0.05,1000,csd_method=method)
        npt.assert_array_almost_equal(c[0,1],c[1,0].conjugate())
...
def test_coherence_regularized():
    ...
    """
    for method in methods:
        f,c = tsa.coherence_regularized(tseries,0.05,1000,csd_method=method)
        npt.assert_array_almost_equal(c[0,1],c[1,0])
...
def test_coherence_phase_delay():
    ...
    for method in methods:
        ...
        npt.assert_almost_equal(tdelay[0,1],pdelay[0,1][1:]/(2*np.pi*f2))
